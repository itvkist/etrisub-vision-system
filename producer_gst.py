...
from GstReader import GStreamerReader
    ...
    def emit_video(self, camera_index, rtsp_url, topic, width=320, height=320, frame_skip=2):
        try:
            ...
            def preprocess_frame():
                # Restrict frame rate produced to ~15 FPS
                #target_fps = 25.0
                ...
                while not self.shutdown_flag.is_set():
                    ...
                    #    continue  # Skip to maintain target FPS

                    #last_frame_time = current_time

                    success, frame = video.read_frame()
                    if not success:
                        logger.warning(f"Failed to read frame from {rtsp_url}: ret: {success}, frame_shape: {None if frame is None else frame.shape}")
                        break
