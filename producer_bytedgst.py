...
from GstReader import GStreamerReader, worker
...
class VideoStreamProducer:
    ...
    def emit_video(self, camera_index, rtsp_url, topic, width=320, height=320, frame_skip=2):
        try:
            ...
            #video.set(cv2.CAP_PROP_FPS, 25)                 # Framerate captured in
            
            video = GStreamerReader(rtsp_url)

            frame_buffer = Queue(maxsize=10)  # Buffer for frame preprocessing            

            # Main sending loop
            while not self.shutdown_flag.is_set():
                success, bytedata = video.read_jpeg_byted()
                if not success:
                    logger.warning(f"Failed to read frame from {rtsp_url}: ret: {success}, frame_shape: {None if bytedata is None else len(bytedata)}")
                    break

                frame_buffer.put(bytedata)

                try:
                    frame_data = frame_buffer.get(timeout=10.0)
